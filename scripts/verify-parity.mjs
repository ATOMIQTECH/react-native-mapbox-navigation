...
const ANDROID_MODULE = 'android/src/main/java/expo/modules/mapboxnavigation/MapboxNavigationModule.kt'
 ...
 *
 * `onDestinationPreview` / `onDestinationChanged`: both are exported from JS as
 * `addDestinationPreviewListener` / `addDestinationChangedListener` and are
 * declared and emitted on Android, but appear nowhere in `ios/` at all. They
 * have therefore never fired on iOS. They originate in the Android Drop-In
 * preview flow (`view.api.startDestinationPreview` / `startRoutePreview`),
 * which is exactly the layer being rebuilt for v3 — so the iOS side should be
 * implemented as part of that work rather than deferred again.
 */
...
for (const prop of missingOnIos) {
  if (!PLATFORM_SPECIFIC_PROPS.android.has(prop)) {
    fail(`Prop "${prop}" is declared on Android but not iOS. Add it, or list it in PLATFORM_SPECIFIC_PROPS.`)
  }
}

for (const prop of missingOnAndroid) {
  if (!PLATFORM_SPECIFIC_PROPS.ios.has(prop)) {
    fail(`Prop "${prop}" is declared on iOS but not Android. Add it, or list it in PLATFORM_SPECIFIC_PROPS.`)
  }
}
...
for (const event of extractJsSubscribedEvents(js)) {
  if (!iosEvents.has(event)) {
    fail(`JS subscribes to "${event}" but iOS declares no such event — the listener would never fire.`, event)
  }
  if (!androidEvents.has(event)) {
    fail(`JS subscribes to "${event}" but Android declares no such event — the listener would never fire.`, event)
  }
}
